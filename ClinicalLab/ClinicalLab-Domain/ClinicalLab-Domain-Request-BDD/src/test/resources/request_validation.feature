Feature: Request Validation
  ...
  Scenario: Requesting blood tests with health insurance
  	Given a "new" client with "health insurance"
		And the lab staff selects "T3", "Cholesterol", and "Glucose" tests for the client
		When the lab staff confirms the test request
   	...
   	And the system notifies the lab staff that the request is registered successfully
  
  
  Scenario: Requesting blood tests with private payment
		Given a "new" client with "private payment"
		And the lab staff selects "Glucose", "T3", "T4", "Triglycerides", "Blood count", "C-reactive protein" tests for the client
		When the lab staff confirms the test request
	...
	Scenario: Modifying the blood test request
		...
		When the lab staff updates the request
		...
		And the system notifies the lab staff that the request has been updated successfully
	...
	Scenario: Completing payment and joining the blood draw waiting list
		...
		And the lab staff selects "private payment" and completes the payment for the client
	...
	Scenario: Viewing the blood test request and confirming that the tests were collected
		...
		When the lab staff accesses the "Client Requests" section
		Then the system displays the receipt with the "exam request number", "client name", and "exame date", "total of blood tests"
		And the lab staff can confirm that the tests were collected successfully
	...
	Scenario: Error when trying to register the same test request twice
		...
		When the lab staff tries to register the same test request again for the client
